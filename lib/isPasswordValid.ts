...
export function isPasswordValid(password: string, breakdown?: boolean, strict?: boolean) {
  ...
  let low = false // Has lowercase characters
  let num = false // At least one number
  let min = false // Eight characters, or fifteen in strict mode.
  let admin_min = false
  if (password.length >= 7 && (!strict || password.length > 14))
    min = true
  if (strict && password.length > 14)
    admin_min = true
  if (password.match(/\d/))
    num = true
